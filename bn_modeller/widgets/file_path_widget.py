...
from PySide6.QtWidgets import QWidget, QHBoxLayout, QLineEdit, QPushButton, QStyle, QFileDialog
...
class FilePathWidget(QWidget):
    ...
    def __init__(self,
        ...
        self._dialogFcn = QFileDialog.getSaveFileName if mode == FilePathWidget.FilePathMode.SaveFileName else QFileDialog.getOpenFileName
        self._init_ui()
    ...
    @Slot()
    def selectFile(self):
        fileName = self._dialogFcn(self,
                                   self._caption,
                                   self._defaultDir,
                                   self._fileSelectionFilter)
        if len(fileName[0]) > 0:
